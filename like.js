...
async function like(i, cookie, token, tweetId, username) {
  for (let retries = 0; retries < MAX_RETRIES; retries++) {
    try {
        ...
        const data = {"variables":{"tweet_id":tweetId},"queryId":"lI07N6Otwv1PhnEgXILM7A"}
        ...
        const requestPromise = axios.post('https://x.com/i/api/graphql/lI07N6Otwv1PhnEgXILM7A/FavoriteTweet', data, {
          headers,
          httpsAgent: agent,
        ...
        if (result && result.data && result.data.data) {
          const status = result.data.data.favorite_tweet;
          if (status !== 'Done') {
            throw new Error(`notlike X ${i}`);
          }
          console.log(`i_${i} liked`);
          return;
        } else {
          throw new Error(`Invalid response structure for X ${i}`);
        }
      } catch (error) {
        if (error.message.includes('timeout') || error.message.includes('notlike')) {
